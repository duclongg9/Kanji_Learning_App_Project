...
class LearningRepositoryImpl @Inject constructor(
    ...
    override fun observeCourses(): Flow<List<CourseItem>> =
        flow { emit(dataSource.loadCourses()) }.flowOn(Dispatchers.IO)
    ...
    override fun observeCourseDetail(courseId: Long): Flow<CourseDetail> =
        flow { emit(dataSource.loadCourseDetail(courseId)) }.flowOn(Dispatchers.IO)
    ...
    override fun observeLessonDetail(lessonId: Long): Flow<LessonDetailModel> =
        flow { emit(dataSource.loadLessonDetail(lessonId)) }.flowOn(Dispatchers.IO)
    ...
    override suspend fun unlockCourseWithMomo(
        courseId: Long,
        phoneNumber: String
    ): PaymentReceipt = withContext(Dispatchers.IO) {
        dataSource.unlockCourse(courseId, phoneNumber)
    }
     ...
     */
    override suspend fun loadQuiz(lessonId: Long): QuizSession = withContext(Dispatchers.IO) {
        dataSource.loadQuizSession(lessonId)
    }
    ...
    override suspend fun recordQuizResult(lessonId: Long, score: Int, total: Int) {
        withContext(Dispatchers.IO) {
            dataSource.recordQuizResult(lessonId, score, total)
        }
    }
}
